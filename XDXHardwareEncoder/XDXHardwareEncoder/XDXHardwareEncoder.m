...
#define XDXResolutionW 2160
#define XDXResolutionH 3840
#define XDXFPS         60
...
- (void)initSaveVideoFile{
    ...
    NSString *savePath           = [path stringByAppendingString:@"/test0.265"];
...
- (void)prepareForEncode {
    ...
    if (self.enableH265) {
        if (@available(iOS 11.0, *)) {
//            HEVC全称High Efficiency Video Coding（高效率视频编码），是比H.264更加优秀的一种视频压缩标准（也称为H.265）
            BOOL hardwareDecodeSupported = VTIsHardwareDecodeSupported(kCMVideoCodecType_HEVC);
            if (hardwareDecodeSupported) {
